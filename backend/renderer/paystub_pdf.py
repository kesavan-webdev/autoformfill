...
def _draw_header(c: canvas.Canvas, data: dict[str, Any]) -> None:
    ...
    c.drawString(325, 748, "Earnings Statement")

    # ADP badge (top right).
    _draw_adp_badge(c, 520, 748)
...
def _draw_table(
    ...
    y -= 2
    line_start = col_x[0] if aligns[0] == "left" else x
    c.line(line_start, y, right_edge, y)
    y -= 9
    c.setFont("Helvetica", 8)
    for row in rows:
...
def _draw_body(c: canvas.Canvas, data: dict[str, Any]) -> None:
    ...
    ly -= 4
    gross_left = lx + 40
    gross_right = e_cols[3]
    c.line(gross_left, ly + 8, gross_right, ly + 8)
    c.setFont("Helvetica-Bold", 8)
    ...
    net_left = lx + 40
    net_right = e_cols[3]
    c.line(net_left, ly + 8, net_right, ly + 8)
    c.setFont("Helvetica-Bold", 8)
    ...
    ry = 542.0   # roughly level with the Statutory Deductions header on the left
...
def _draw_federal_wages(c: canvas.Canvas, data: dict[str, Any]) -> None:
    ...
    if fed is None:
        return
    c.setFont("Helvetica", 8)
    c.drawRightString(555.0, 220.0, f"Your federal taxable wages this period are  {_fmt_dollar(fed)}")
...
def _draw_tearoff(c: canvas.Canvas, data: dict[str, Any]) -> None:
    ...
    text = "THIS IS NOT A CHECK"
    prefix_w = c.stringWidth("THIS IS NOT ", "Helvetica-Bold", 30)
    a_w = c.stringWidth("A", "Helvetica-Bold", 30)
    c.translate(340 - prefix_w - a_w / 2, 75)
    c.rotate(9)
    # Stroke-only text with a fine dotted dash pattern so the letters read
    # as dot-outlined characters (like a real check watermark).
    c.setStrokeGray(0.5)
    c.setFillGray(0.78)
    c.setLineWidth(0.6)
    c.setDash([0.4, 1.2], 0)
    t = c.beginText(0, 0)
    t.setTextRenderMode(2)  # fill and stroke — filled letters with dotted outline
    t.setFont("Helvetica-Bold", 30)
    t.textOut(text)
    c.drawText(t)
    c.restoreState()
    ...
    c.drawString(300, 180, "Pay Date:")
    c.setFont("Helvetica", 9)
    c.drawString(370, 180, _s(data.get("payDate")))
